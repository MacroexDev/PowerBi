select 
  ...
  case 
    when T1.closedate is null  then 'Não Fechado'
    when T1.closedate <=  date_add(T1.solvedate, INTERVAL 6 day)  then 'Não'
    else 'Sim'
  end 'Fechado Aut.',
  case 
  ...
  replace(replace(replace(replace(replace(replace(T10.content,'&lt;p&gt;',' ' ),'&lt;/p&gt;',' '),'&gt;',' '),'&lt;',' '),'/span',''),'span','')  'Solucao',
  T2.users_id 'users_id_request', T5.realname 'Usuario Abertura', 
  T3.users_id 'users_id_ti', T6.realname 'Tecnico Resp.',
  ifnull(T7.satisfaction,0) 'Nota',  ifnull(T7.comment,'') 'Avaliacao'
