select 
  T1.id 'id_ticket', T1.name 'Titulo', T1.date 'Data Abertura', 
  IFNULL(T1.closedate,'1900-12-31 00:00:00') 'Data Fechamento', 
  T1.solvedate 'Data Sol.', T1.date_mod 'Data Ult. Alt.',
  IFNULL(T1.time_to_resolve,'1900-12-31 00:00:00') 'Data Prev.',
  case 
    when T1.solvedate is null then DATEDIFF(now(),T1.date)
    else DATEDIFF(T1.solvedate,T1.date)
  end 'Dias Solução',
  case 
    when T1.closedate is null then DATEDIFF(now(),T1.date)
    else DATEDIFF(T1.closedate,T1.date)
  end 'Dias Fechamento',
  case 
  ...
  T1.users_id_lastupdater, T4.name 'Usuario Ult. Alt.',  
  ...
  case  
    when T12.projects_id > 0 then T13.name
	else ' - '
  end 'Nome Projeto', 
  ifnull(T14.name,' - ') 'Localização',
  T11.name 'Origem',
  replace(replace(replace(replace(replace(replace(T10.content,'&#60;p&#62;',' ' ),'&lt;p&gt;',' ' ),'&lt;/p&gt;',' '),'=&gt;',' '),':&lt;',' '),'em&gt;',' ')  'Solucao', 
  T2.users_id 'users_id_request', T5.name 'Usuario Abertura', 
  T3.users_id 'users_id_ti', T6.name 'Tecnico Resp.',
  ...
  ifnull(T15.groups_id,0) as 'idGroup',  ifnull(T16.name,' - ') as 'Grupo Resp.',
  case 
    when ifnull((select count(T21.id ) from glpi_tickettasks T21
			where T21.tickets_id = T1.id
  			  and T21.taskcategories_id = 1),0) > 0 then 'Sim'
  	else 'Não'
  end 'Sup. Fora SLA',
  T1.entities_id, T17.name as 'Empresa'
from glpi_tickets T1
left join glpi_tickets_users T2 on T1.id = T2.tickets_id and T2.type = 1
left  join glpi_tickets_users T3 on T1.id = T3.tickets_id and T3.type = 2
...
left join glpi_projects T13 on T13.id = T12.projects_id 
left join glpi_locations T14 on T14.id = T1.locations_id 
left join glpi_groups_tickets T15 on T15.tickets_id = T1.id
  and T15.type = 2
left join glpi_groups T16 on T16.id = T15.groups_id
left join glpi_entities T17 on T1.entities_id = T17.id
