select 
  ...
  case 
    when T1.solvedate is null then DATEDIFF(T1.date,now())
    else DATEDIFF(T1.date,T1.solvedate)
  end 'Dias Solução',
  case 
    when T1.closedate is null then DATEDIFF(T1.date,now())
    else DATEDIFF(T1.date,T1.closedate)
  end 'Dias Fechamento',
  case 
  ...
  case 
    when ifnull((select count(T21.id ) from glpi_tickettasks T21
			where T21.tickets_id = T1.id
  			  and T21.taskcategories_id = 1),0) > 0 then 'S - Sim'
  	else 'N - Não'
  end 'Sup. Fora SLA'
